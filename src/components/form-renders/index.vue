<template>
  <div class="form-renders">
    <el-form class="form-container" :model="formEvents.forms" :labelWidth="formEvents.props.labelWidth || '80px'" v-bind="formEvents.props"
      :ref="el => formEvents.formsRef = el" @validate="(...args:any) => emit('validate', ...args)">
      <el-row :gutter="formEvents.layout.gutter || 20" class="row">
        <el-col v-for="item in formEvents.renderList || []" :span="item.span || formEvents.layout.span || 6"
          class="col">
          <el-form-item v-if="!item.hide || true" v-bind="item.formItemsProps || {}" :label="item.label" :prop="item.prop"
...
const loaderEvents: any = {
  ...
  rendersLoader: () => { // 整个 forms 进行loader[入口]
    ...
    formEvents.layout = props.renders.layout ? JSON.parse(JSON.stringify(props.renders.layout)) : {} // 需要向 el-form 组件中插入的 layout
    props.renders.list.forEach((item: any) => { // 插入 events 列表
      formEvents.renderListEvents[item.prop] = item.events ? { ...item.events } : {}
      if (item.default) { // 注入default默认值
  ...
  slotsListEventsLoader: (): any => { // 插槽装载[未使用的插槽不会进行注册]
    ...
    formEvents.renderList.forEach((renderItem: any) => {
      ...
      if (renderItem.slots.constructor == String) {
        ...
        console.error('slots only accepts string, Object, in prop :' + renderItem.props)
      }
    })
...
const selfEvents = { // 自身方法
  getEventsSubs(infos: any) {
    if (formEvents.renderListEvents[infos.prop][infos.eventName] && typeof formEvents.renderListEvents[infos.prop][infos.eventName] === 'function') {
...
const exportEvents = { // 需要暴露的属性与方法
  getFormsValue() {
    return JSON.parse(JSON.stringify(formEvents.forms))
  },
  validate: (...args: any) => { formEvents.formsRef.validate(...args) },
  validateField: (...args: any) => { formEvents.formsRef.validateField(...args) },
  resetFields: (...args: any) => { formEvents.formsRef.resetFields(...args) },
  scrollToField: (...args: any) => { formEvents.formsRef.scrollToField(...args) },
  clearValidate: (...args: any) => { formEvents.formsRef.clearValidate(...args) },
  setValue: (key: string, value: any) => { formEvents.forms[key] = value}
}
...
const install = () => {
  ...
  if (!props.passivity && props.modelValue) {
    console.error('error from form-renders, props modelValue[v-model] only available when props passivity is true')
