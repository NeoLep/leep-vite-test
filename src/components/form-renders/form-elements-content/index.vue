<template>
  <div class="forms-element-content">
    <div v-if="!componentLists['form-'+ props.renders?.type]">
      could not find component named <span style="color: red">{{ 'form-'+ props.renders?.type }}</span>
    </div>
    <component v-else :is="componentLists['form-'+ props.renders?.type]" v-model="modelValue"
      v-bind="props.renders?.props || {}" @update:modelValue="(val: any) => emit('update:modelValue', val)"
      :renders="renders" :slotsListOrign="slotsList" :slotsListTransform="slotsEvents.slotsTransform(slotsList)"
...
import { slotsTransformResultObjects } from './form-elements-content.interface'
...
for (let key in itemsList) {
  ...
  componentLists[injectKey] = defineAsyncComponent(() => import(`./${injectKey}/index.vue`))
}

// 事件集合
const subEvents = {
  getEvents: (types: string, refIs: any, getArgs: any) => {
    if (!props.renders?.prop) console.error('could not found prop in this render', props.renders)
    const sendProtos = {
...
const slotsEvents = {
  slotsTransform: (slotsList: any): any => {
    let resultObjects: slotsTransformResultObjects = {
      slotPropsList: [],
      slotRenamedList: []
    }
    for (let key in slotsList) {
